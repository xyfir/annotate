...
For now, auto-annotator requires you to have Node and npm installed, and have some familiarity with using the command line. In the future, an optional GUI interface bundled in an installer may exist.
...
- Generate an access key for your Xyfir Annotations account and save it to auto-annotator's config.
  - Go [here](https://annotations.xyfir.com/app/#/users/access-keys).
  - Generate a new key.
  - Copy that key.
  - Run the following command: `auto-annotator config --key xyfirAnnotationsAccessKey --value "ACCESS_KEY"`.
- Save your Calibre ebook library path to config.
  ...
  - Run the following command: `auto-annotator config --key calibreLibraryPath --value "LIBRARY_PATH"`.
- Save Calibre's binary directory path to config.
  ...
  - Run the following command: `auto-annotator config --key calibreBinPath --value "BIN_PATH"`.
